# 🤖 Chatty AI - Desktop Chatbot

A simple desktop-based AI chatbot built using Python with a clean GUI interface. Chatty AI can respond to greetings, tell jokes, share fun facts, display the current time/date, provide weather information, and even generate playful flirt responses.


---

## ✨ Features

- 🕒 Current Time & Date
- 😂 Random Jokes
- 🎯 Fun Facts
- 💬 Greeting Responses
- ❤️ Flirt Mode
- 🌦️ Weather Information
- 🖥️ Simple Desktop GUI
- ⚡ Fast Response System

---

## 📸 Preview

### Main Interface

---

## 🛠️ Tech Stack

- Python
- Tkinter
- Requests
- JSON
- APIs (Weather Integration)

---

## 📂 Project Structure

```bash
AI-CHAT-BOT/
│
├── assets/
│   └── chatty-ai.png
│
├── chatbot.py
├── requirements.txt
├── README.md
└── data/
```

---

## 🚀 Installation

### Clone Repository

```bash
git clone https://github.com/10ASB/AI-CHAT-BOT.git
cd AI-CHAT-BOT
```

### Install Dependencies

```bash
pip install -r requirements.txt
```

### Run Application

```bash
python chatbot.py
```

---

## 🎮 Available Commands

| Command | Description |
|----------|------------|
| Hello | Greeting |
| Time | Shows current time |
| Date | Shows current date |
| Fun fact | Random fact |
| Tell me a joke | Random joke |
| Flirt with me | Fun flirting response |
| Weather at loc | Weather information |
| Bye | Exit chatbot |

---

## 🌟 Future Improvements

- Voice Assistant Support
- AI/LLM Integration
- Better UI Design
- Theme Switching
- Chat History Storage
- Speech-to-Text

---

## 👨‍💻 Author

**Akshay S Bharadwaj**

AI & ML Engineering Student

GitHub: https://github.com/10ASB

---

## 📸 Application Preview
<img width="1920" height="1080" alt="image" src="https://github.com/user-attachments/assets/8a195f2a-dc33-4aa2-b26c-9e9fcc93c9e4" />


The screenshot below shows the Chatty AI desktop chatbot interface. Users can interact with the chatbot through predefined commands such as greetings, jokes, fun facts, time/date queries, weather updates, and playful conversations. The application features a modern dark-themed GUI designed for a smooth and user-friendly experience.



## 📜 License

This project is open-source and available under the MIT License.
